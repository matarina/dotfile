(function (global, factory) {
  ...
  ArkGlobalContext._setViewTemplate('pic', `
...
`);
  ...
  ArkGlobalContext._setViewTemplate('contact', `
<View id="contact" size="500,112" metadatatype="contact">
    ...
    <Text id="nickname" value="&#x6635;&#x79F0;" metadatatype="nickname" size="100, 25" anchors="7" margin="80,22,12,0" ellipsis="true" font="app.16" textcolor="0xFF000000"></Text>
...
<View id="music" size="500,113" metadatatype="music">
    ...
    <Text id="title" margin="12,22,84,0" anchors="7" size="200,20" textcolor="0xff03081A" value="&#x6807;&#x9898;" font="app.17" ellipsis="true" maxline="1"></Text>
    <Text id="desc" margin="12,46,84,0" anchors="7" size="200,17" textcolor="0xff878B99" value="&#x5185;&#x5BB9;" font="app.14" ellipsis="true" maxline="1"></Text>
...
`);
  ...
  ArkGlobalContext._setViewTemplate('miniapp', `
...
local MAX_PIC_HEIGHT = 200
local FAIL_IMAGE_RES = "res/images/pic-default.png"
local FAIL_IMAGE_WIDTH = 23
local FAIL_IMAGE_HEIGHT = 23
...
function ViewModel:Initialize(view)
    ...
    self.previewView = self.view:GetUIObject("previewView")
    self.previewContentView = self.view:GetUIObject("previewContentView")
    self.previewBackground = self.previewContentView:GetTexture("previewBackground")
    self.tag = self.view:GetUIObject("tag")
    self.tagIcon = self.view:GetUIObject("tagIcon")
    self.tagView = self.view:GetUIObject("tagView")
    self.tagBackground = self.tagView:GetTexture("tagBackground")
    self.jumpUrl = ""
    
    self.maxPicWidth, _ = self.view:GetSize()
    self.maxPicHeight = MAX_PIC_HEIGHT;
    self.previewView:SetSize(150, MAX_PIC_HEIGHT) -- 150,200 %E8%BF%99%E4%B8%AA%E5%80%BC%E6%9D%A5%E8%87%AA%E4%BA%8E%E8%AE%BE%E8%AE%A1%E6%90%9E
    
    self.preview:SetValue(theme.getLoadingImage())
    self:ApplyTheme()
    
    utils.log("pic: max-pic-size=" .. self.maxPicWidth .. "," .. self.maxPicHeight)
end
...
function ViewModel:UpdateStyle()
    ...
    if self.isUpdatingStyle then
        utils.log("update style: in progress")
        return
    end
    
    if not self.hasSetMetaData then
        utils.log("update style: data not initialize")
        return
    end
...
function ViewModel:DoUpdateStyle()
    utils.log("pic: update style, view-style=" .. self.view:GetStyle())
    
    self:UpdatePreview(self.previewUrl)
    self:UpdateTag()
    self:ApplyTheme()
   
    flexutil.SetStyle(self.view, "width", "auto")
    flexutil.SetStyle(self.view, "height", "auto")
    
    
end

function ViewModel:UpdateTag()
    
    if not qqutil.IsInAIO() then
        self.tagView:SetVisible(false)
    end
end

function ViewModel:OnResize(width, height)
    utils.log("pic: onResize: size=" .. width .. "," .. height)
    
    
    if not self.hasSetMetaData then
        self.maxPicWidth = width 
        flexutil.SetStyle(self.previewView, "width", tostring(width))
        utils.log("pic: update max pic size, size=" .. self.maxPicWidth .. "," .. self.maxPicHeight)
        return
    end
    
    if self.tryUpdateSizeCount > 5 then
        utils.log("pic: onResize: too many resize")
        return
    end
...
function ViewModel:OnSetMetadata(value)
    ...
    if value and value["pic"] then
        local data = value["pic"]
        
        self.jumpUrl = utils.fixurl(data["jumpUrl"], self.jumpUrl)
        self.previewUrl = utils.fixurl(data["preview"], self.previewUrl)
        
        self.preferPicWidth = 0
        if data["width"] then
            self.preferPicWidth = data["width"]
        end
        
        self.preferPicHeight = 0
        if data["height"] then
            self.preferPicHeight = data["height"]
        end
        ...
        if data["tag"] == nil then
            ...
            if self.tagIconUrl then 
                ...
                flexutil.SetStyle(self.tag, "marginLeft", "7");
            end
        end
    end
...
function ViewModel:UpdatePreview(previewUrl)
    if previewUrl == nil then
        utils.log("pic: preview uri is nil")
        return
    end
    ...
    local stylePattern = "display:flex;width:%d;height:%d;flexDirection:column;alignItems:center;justifyContent:center"
    
    if self.preferPicWidth > 0 and self.preferPicHeight > 0 then
        local picWidth, picHeight, mode = self:ComputePicSize(
            self.preferPicWidth, self.preferPicHeight, self.maxPicWidth, self.maxPicHeight)
            
        self.preview:SetMode(mode)
        self.previewView:SetStyle(string.format(stylePattern, picWidth, picHeight))
        
        local preview = self.preview
        self.preview:AttachEvent("OnLoad", function()
            preview:DetachEvent("OnLoad")
            
            preview:SetStyle("display:flex;width:100%;height:100%")                     
        end)
        
        utils.SetImageValue(self.preview, previewUrl, FAIL_IMAGE_RES, function(isSuccess)
            if not isSuccess then
                preview:SetStyle(string.format("display:flex;width:%d;height:%d", 
                    FAIL_IMAGE_WIDTH, FAIL_IMAGE_HEIGHT))          
            end
        end)
        
        utils.log("pic: prefer-size=" .. self.preferPicWidth .. "," .. self.preferPicHeight
                .. ", preview-size=" .. picWidth .. "," .. picHeight .. ", mode=" .. mode)
    else
        
        local image = UI.Image()
        image:AttachEvent("OnLoad", function()
            image:DetachEvent("OnLoad")
            self.preferPicWidth, self.preferPicHeight = image:GetSize()
            
            local picWidth, picHeight, mode = self:ComputePicSize(
                self.preferPicWidth, self.preferPicHeight, self.maxPicWidth, self.maxPicHeight)
            
            self.preview:SetMode(mode)
            self.preview:SetStyle("display:flex;width:100%;height:100%")
            self.previewView:SetStyle(string.format(stylePattern, picWidth, picHeight))
            utils.SetImageValue(self.preview, previewUrl, FAIL_IMAGE_RES)
            
            utils.log("pic: prefer-size=" .. self.preferPicWidth .. "," .. self.preferPicHeight
                .. ", preview-size=" .. picWidth .. "," .. picHeight .. ", mode=" .. mode)
        end)
        utils.SetImageValue(image, previewUrl, FAIL_IMAGE_RES)
    end
end

function ViewModel:ComputePicSize(picWidth, picHeight, maxWidth, maxHeight)

    local mode = "aspectfill"
    
    if picWidth >= picHeight * 3 then
        picWidth = picHeight * 3
    elseif picHeight >= picWidth * 3 then
        picHeight = picWidth * 3;
    end
    
    
    if picHeight <= maxHeight then
        
        if picWidth <= maxWidth then
            return picWidth, picHeight, mode
        end

        local adjustWidth = maxWidth;
        local adjustHeight = math.floor(maxWidth * picHeight / picWidth)
        return adjustWidth, adjustHeight, mode
    end
    
    
    local adjustHeight = maxHeight;
    
    local adjustWidth = math.floor(maxHeight * picWidth / picHeight)
    
    adjustWidth, adjustHeight, _ = self:ComputePicSize(adjustWidth, adjustHeight, maxWidth, maxHeight);
    return adjustWidth, adjustHeight, mode    
end
...
function ViewModel:ApplyTheme() 
    ...
    uiutil.SetColor(self.tag, colorConfig[theme.THEME_COLOR_TAG])
    uiutil.SetColor(self.tagBackground, colorConfig[theme.THEME_COLOR_TAG_BACKGROUND])
    uiutil.SetColor(self.previewBackground, colorConfig[theme.THEME_COLOR_PIC_BORDER])
end

function ViewModel:OnConfigChange(config)
    utils.log("pic OnConfigChange appconfig="..Net.TableToJSON(config))
    self:ApplyTheme()
end 

function ViewModel:OnStartup(config)
    utils.log("pic OnStartup appconfig="..Net.TableToJSON(config))
end
          _G['pic'] = {
            MAX_PIC_HEIGHT = MAX_PIC_HEIGHT,FAIL_IMAGE_RES = FAIL_IMAGE_RES,FAIL_IMAGE_WIDTH = FAIL_IMAGE_WIDTH,FAIL_IMAGE_HEIGHT = FAIL_IMAGE_HEIGHT,ViewModel = ViewModel
          }
        `;
...
function ViewModel:SetIcon(view, imageType)
    ...
    if imageType ~= BtnType.TYPE_LOADING then
        ...
        canvas:DrawImage(image, left, top, iconWidth, iconHeight)
...
function ViewModel:Initialize(view)
    ...
    self.nickname = view:GetUIObject("nickname")
    self.avatar = view:GetUIObject("avatar")
    self.tag = view:GetUIObject("tag")
    self.tagIcon = view:GetUIObject("tagIcon")
...
function ViewModel:UpdateStyle(width, height)
   ...
   self.isUpdatingStyle = true
   self:DoUpdateStyle(width, height)
   self.isUpdatingStyle = false
end

function ViewModel:DoUpdateStyle(width, height)
    utils.log("DoUpdateStyle%EF%BC%8C size=" .. width .. "," .. height)
    self.view:SetSize(width, DEFAULT_VIEW_HEIGHT)
    self:ApplyTheme()
end

function ViewModel:OnResize(width, height)
    if not self.hasSetMetaData then
        if qqutil.IsInAIO(self.view) then
            self.width = uiutil.AdjustViewWidth(width)
        else
            self.width = width
        end
        utils.log("update view width: " .. width .. "->" .. self.width)
        return
    end

    utils.log("contact: OnResize, size=" .. width .. "," .. height
...
function ViewModel:OnClick(sender)
	local id = sender:GetID()
    if sender == sender:GetRoot() then
        local deskUrl = 'ntqq://arkJump?url='..utils.urlEncode(self.jumpUrl);
        utils.loadUrl(deskUrl, self.view:GetRoot())
        return
    end
end
...
function ViewModel:OnSetMetadata(value)
    ...
    utils.log("contact: OnSetMetadata")
    self.hasSetMetaData = true
	...
	if value and value["contact"] then
        ...
        self.avatarUrl = utils.fixurl(data["avatar"], self.avatarUrl)
        self.jumpUrl = utils.fixurl(data["jumpUrl"], self.jumpUrl)

        if self.avatarUrl then
            utils.SetImageValue(self.avatar, self.avatarUrl, "res/images/contact-default.png")
        end
...
function ViewModel:ApplyTheme()
    ...
    uiutil.SetColor(self.nickname, colorConfig[theme.THEME_COLOR_TITLE])
    uiutil.SetColor(self.contactInfo, colorConfig[theme.THEME_COLOR_SUMMARY])
    uiutil.SetColor(self.tag, colorConfig[theme.THEME_COLOR_SOURCE])
    uiutil.SetColor(self.seperatorColor, colorConfig[theme.THEME_COLOR_SEPERATOR])
end
...
function ViewModel:OnConfigChange(config)
    ...
    self:ApplyTheme()
          ...
          _G['contact'] = {
            DEFAULT_VIEW_HEIGHT = DEFAULT_VIEW_HEIGHT,ViewModel = ViewModel
          }
        `;
...
local SINGLE_LINE_MARGIN = 4

local DEFAULT_PIC_WIDTH = 23
local DEFAULT_PIC_HEIGHT = 23
...
function ViewModel:Initialize(view)
    ...
    self.tag = view:GetUIObject("tag")
    self.tagIcon = view:GetUIObject("tagIcon")
    self.title = view:GetUIObject("title")
    self.titleView = view:GetUIObject("titleView")
    self.preview = view:GetUIObject("preview")
    self.previewView = view:GetUIObject("previewView")
    self.bottom = view:GetUIObject("bottom")
    
    
    self:SetPreviewImage(theme.getLoadingImage(), DEFAULT_PIC_HEIGHT, DEFAULT_PIC_HEIGHT) 
end

function ViewModel:SetPreviewImage(value, width, height)
...
function ViewModel:DoUpdateStyle(width, height)
    utils.log("miniapp: DoUpdateStyle, size=" .. width .. "," .. height)

    local contentMarginLeft = 12
    local contentMarginRight = 12
    
    local MULTI_LINE_HEIGHT = 20
    
    local SINGLE_LINE_HEIGHT = MULTI_LINE_HEIGHT + SINGLE_LINE_MARGIN
       
    
    local titleWidth = width - contentMarginLeft - contentMarginRight
    local textWidth, _ = textutil.measureText(self.title:GetValue(), self.title:GetFont())
    if textWidth > titleWidth then
        if utils.isIOS() then
            self.title:SetLineHeight(MULTI_LINE_HEIGHT)
        end
        self.isTitleSingleLine = false
        self.titleView:SetSize(titleWidth, MULTI_LINE_HEIGHT * 2)
    else
        self.isTitleSingleLine = true
        self.titleView:SetSize(titleWidth, SINGLE_LINE_HEIGHT)
    end
    
    
    self:UpdatePreviewSize(width, height)
    
    
    local _, _, _, bottomEnd = self.bottom:GetRect()
    self.view:SetSize(width, bottomEnd)
...
function ViewModel:UpdatePreviewSize(parentWidth, parentHeight)

    local width = parentWidth - 12 * 2 
    local height = math.floor(width * 4 / 5)
    self.previewView:SetSize(width, height)
    ...
    local w, h = self.previewView:GetSize()
    utils.log("miniapp: update preview size: " .. width .. "," .. height
        .. ", real-size:" .. w .. "," .. h)
end

function ViewModel:OnResize(width, height)
    utils.log("miniapp: OnResize, size=" .. width .. "," .. height)
    if not self.hasSetMetaData then
        if qqutil.IsInAIO(self.view) then
            self.width = uiutil.AdjustViewWidth(width)
        else
        ...
        utils.log("miniapp: update view size, size=" .. self.width .. "," .. self.height)
        return
    end
...
function ViewModel:OnSetMetadata(value)
	...
	if value and value["miniapp"] then    
        local data = value["miniapp"]
        self.meta = data
        ...
        if data["tagIcon"] then
            utils.SetImageValue(self.tagIcon, data["tagIcon"], "res/images/icon-default.png")
        else
            self.tagIcon:SetVisible(false)
            self.tag:SetMargin(0,0,0,0)
        end
        ...
        local _self = self
        utils.SetImageValue(self.preview, data["preview"], "res/images/pic-default.png", function(success)
            if not success then
                self:SetSize(DEFAULT_PIC_WIDTH, DEFAULT_PIC_HEIGHT) 
                utils.log("miniapp: load preview failed, url=" .. data["preview"])
                return
            end
            
            _self.preview:SetAnchors(15)
            _self.preview:SetMargin(0,0,0,0)
        end)
        
        self.jumpUrl = utils.fixurl(data["jumpUrl"], self.jumpUrl)
    end
    
    utils.log("miniapp: update style, size=" .. self.width .. "," .. self.height)
    self:UpdateStyle(self.width, self.height)
end

function ViewModel:OnConfigChange(config)
    utils.log("miniapp: OnConfigChange appconfig="..Net.TableToJSON(config))
    utils.setBackground(self.view, app.getThemeConfig())
end 

function ViewModel:OnStartup(config)
    utils.log("miniapp: OnStartup appconfig="..Net.TableToJSON(config))
end
          _G['miniapp'] = {
            SINGLE_LINE_MARGIN = SINGLE_LINE_MARGIN,DEFAULT_PIC_WIDTH = DEFAULT_PIC_WIDTH,DEFAULT_PIC_HEIGHT = DEFAULT_PIC_HEIGHT,ViewModel = ViewModel
          }
        `;
...
function MusicPlayer:ProcessDict(tag, dict)
    utils.log("ProcessDict dict = "..tostring(dict))
	local newSeq, newUrl
	if type(dict)=="table" then
		...
		if dict.url and dict.url~="" then
			...
			if type(cache)=="table" then
				...
				utils.log(string.format("%s.%s.Can't get seq from url.%s", app.GetAppName(), tag, dict.url))
			end
		end
	else
		utils.log(string.format("%s.%s.dict is nil", app.GetAppName(), tag))
	end
...
function MusicPlayer:RawStop()
	...
	self.currSeq = nil
    self.currUrl = nil
    utils.log(string.format("RawStop self.currUrl:%s",self.currUrl))
	self:OnStateChange()
end
...
function MusicPlayer:RawPause()
	...
	self.state = MusicPlayer.STATE_PAUSE
        utils.log(string.format("RawPause self.currUrl:%s",self.currUrl))
	self:OnStateChange()
end
...
function MusicPlayer:RawResume()
	...
	self.state = MusicPlayer.STATE_PLAYING
        utils.log(string.format("RawResume self.currUrl:%s",self.currUrl))
	self:OnStateChange()
end
