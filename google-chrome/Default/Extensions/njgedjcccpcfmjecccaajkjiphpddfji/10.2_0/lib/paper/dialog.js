(function($){
	$.fn.dailog = function(options,callBack){
	    ...
	    let opt = $.extend(defaultDailog,options||{});

	    // 设置btn是否有颜色
	    let btn_className = opt.isBtnHasBgColor?'':'no_bg';
	    ...
	    let _colseScroll = function(){
	    	$('body').css({
	    		overflow:'auto',
	    	});
	    }

	    let _overflowBtn = function(){
	    	// bottons超过两个提示
	    ...
	    let _btnIndex = function(name){
	    	//获取点击的索引
	      	let btnName = name || '';
	      		for(let i = 0;i<opt.bottons.length;i++){
	    ...
	    let _init = function(){
		    ...
		    if(!opt.isInput){
		    	_this.discription_dailog = $("<div class='discription_dailog' id='easyScholar-paper'></div>").html(opt.discription).appendTo(_this.dailog_div);
		    }else{
	    ...
	    //   // // _btnIndex(name);
		//
	    //   // let inputstatus = _this.input_dailog? 1:0;
	    //   // let inputvalue = inputstatus? _this.input_dailog.val():'';
		//
	    //   // 设置返回值
	    //   // let ret = {
	    //   // 	index:btnIndex,
